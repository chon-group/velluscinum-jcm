myWallet("GfB4KA8AbT8nvzE54vgxWtCgE69Cos1BBQEideRqtMxq","J9kiTCvUu81cDUakkpcxuPv1vWgYtiE2Ls8uzxgeSMPn"). /* Auto generated GiacomoWallet - consult startMAS.sh*/
...
+!create_auction_artifact(Task,MaxPrice)
   <- .concat("auction_for_",Task,ArtName);
      ...
      velluscinum.deployNFT(S,P,Q,I,M,registredContract(ArtId)); /* new action */
...
+!prepareContract(ContractID,CompanyPublicKey)[source(Company)]: 
   ...
   velluscinum.transferNFT(S,P,Q,ContractID,CompanyPublicKey,
...
+!payment(ArtID, CompanyPublicKey)[source(Company)]: 
   ...
   velluscinum.transferToken(S,P,Q,C,CompanyPublicKey,Price,paymentTransaction(ArtID));
